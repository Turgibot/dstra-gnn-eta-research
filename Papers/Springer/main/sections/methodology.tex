...
After generation, we remove the longest 5\% of SUMO trips by trip-start duration, using the 95th-percentile cutoff of 2{,}587\,s (43.1\,min). This removes a rare simulator tail of very long trips whose durations are disproportionate to their route lengths and would otherwise dominate aggregate error metrics, while retaining about 94.9\% of SUMO vehicles for training and evaluation. This p95-filtered SUMO corpus is the SUMO dataset used in all reported results.
...
\begin{itemize}
    ...
    \item \textbf{Fusion and MoE Head:} Vehicle embeddings, enriched with route and temporal context, are fused through an MLP (256 hidden, GELU, LayerNorm, dropout 0.1) into a 192-dim representation and routed to a sparse Top-$k$ Mixture-of-Experts (MoE) head (dropout 0.1), shown in the router panel in Fig.~\ref{fig:components}, where specialized experts capture heterogeneous traffic regimes. We route to $k{=}2$ of 6 experts because traffic regimes differ qualitatively across the network and this setting balances per-vehicle specialization against expert-collapse risk while keeping the router tractable at our batch size, as Table~\ref{tab:moe-expert-sweep}'s sensitivity sweep confirms. The output is the ETA prediction for each vehicle.
\end{itemize}
...
\end{equation}
All metrics are computed in seconds. The precise evaluation protocol and population (trip-start predictions on the test split) are defined in Sect.~\ref{sec4}.
