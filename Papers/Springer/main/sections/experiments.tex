...
\noindent Figs.~\ref{fig:sumo-trip-dist}--\ref{fig:porto-trip-dist} show trip-duration and trip-distance distributions by chronological split. The train, validation, and test curves overlap closely in all three datasets, supporting the split choice without visible distributional drift. In Fig.~\ref{fig:sumo-trip-dist}, the duration axis is capped at 6{,}000\,s for readability and the dashed vertical line marks the p95 cutoff at 2{,}587\,s. SUMO statistics and results use the p95-filtered population defined in Sect.~\ref{sec:porto-conversion}; the distribution remains broader than the Porto datasets and has a bimodal distance profile, consistent with distinct intra-zone and inter-zone trip populations. Fig.~\ref{fig:porto-g-vs-t} separately compares the Porto-T and Porto-G populations and their different distance definitions.
...
Headline evaluation uses one prediction per trip start so that graph-based models are compared against trajectory-based baselines on the same unit of prediction: one ETA per trajectory. Porto-G and Porto-T fall within the $[315,945]$\,s protocol range by construction. SUMO is reported in two views: the shared protocol range, and the broader p95-filtered population obtained by removing trips longer than 2{,}587\,s as described in Sect.~\ref{sec:porto-conversion}. All SUMO results outside the protocol-range table use this p95-filtered population.

Training uses the normalized log target defined in Sect.~\ref{sec:loss}. For each dataset and variant, model selection uses the lowest \emph{validation} MAE per seed, with validation reserved exclusively for early stopping and checkpoint selection. All headline numbers are computed on the held-out \emph{test} split: for variants A3--A6, we report the mean $\pm$ std across three seeds (42, 43, 44), while A1 and A2 are reported for seed 42 only.
...
To address the concern that comparisons rely mainly on internal ablations and a single average-time baseline, we retrain and evaluate a ladder of external reference methods on Porto-T (trip-level trajectory input) or Porto-G (graph snapshot input), matching our model's chronological splits and label definition, defined in Sect.~\ref{sec:eval-protocol} and Sect.~\ref{sec:porto-dataset} respectively:
\begin{itemize}
    \item \textbf{AVG}: historical OD-and-hour average travel time, falling back to coarser OD and global averages when a bucket is unseen (MetaTTE-style). Evaluated on Porto-T.
    \item \textbf{Linear Regression (LR)}: ridge-regularized linear model over the same trip-geometry features as GBM. Included as a linear ceiling for the tabular feature set.
    \item \textbf{GBM}: gradient-boosted trees (LightGBM) over trip- and route-geometry features (origin/destination coordinates, polyline distance, OD distance, bearing, hour-of-day, day-of-week). Evaluated on Porto-T.
    \item \textbf{TEMP}: temporal-only regression using hour-of-day and day-of-week features. Serves as an ablation of the time-of-day signal in isolation from spatial features.
    \item \textbf{Route-sum}: sum of per-edge remaining travel times along \texttt{vehicle\_route\_left}, using per-edge historical average speeds from the training split. Evaluated on Porto-G.
    ...
    \item \textbf{DCRNN}~\cite{dcrnn2018}: diffusion convolutional recurrent neural network adapted to the Porto-G road-edge line graph, predicting per-vehicle ETA from the same $H{=}30$ snapshot window. On Porto-G, DCRNN uses a stride of 100 (vs.\ stride 10 for DSTRA-GNN) due to the much larger static graph (79{,}160 road edges), which would otherwise make training infeasible. This yields only $\sim$6{,}800 training windows from the Porto-G training split.
\end{itemize}
DeepTTE, MetaTTE, LR, GBM, TEMP, WDR, and STNN use trip-level Porto-T inputs. AVG, Route-sum, DCRNN, and DSTRA-GNN use Porto-G graph snapshots. SUMO is used for controlled DSTRA-GNN ablations over the same graph schema.
