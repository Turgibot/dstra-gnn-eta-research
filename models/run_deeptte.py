"""Train and evaluate the DeepTTE baseline on Porto-T.

Usage (from repo root):
    uv run --python 3.8 --with torch --with polars --with numpy python models/run_deeptte.py
    uv run --python 3.8 --with torch --with polars --with numpy python models/run_deeptte.py \
        --epochs 100 --batch-size 256 --lr 1e-3 --seed 42 --output-dir results/deeptte
"""
import argparse
import json
import random
...
from models.baselines.deeptte import DeepTTE, PortoTrajDataset, collate_fn
...
_OUTPUT_DIR = _REPO_ROOT / "results/deeptte"
...
def _parse_args():
    ...
    p.add_argument("--patience",    type=int,   default=10,
                   help="Early-stop patience (epochs without val MAE improvement)")
...
def _build_taxi_vocab(train_df) -> dict:
    taxis = train_df["taxi_id"].unique().to_list()
    return {tid: i + 1 for i, tid in enumerate(sorted(taxis))}


@torch.no_grad()
def _evaluate(model, loader, device, label_mean: float, label_std: float) -> dict:
    model.eval()
    y_true, y_pred = [], []
    for coords, cdist, attr_ids, dist_n, labels, lengths, raw_labels in loader:
        ...
        preds_norm = model(coords, cdist, attr_ids, dist_n, lengths)
        preds = preds_norm.cpu().numpy() * label_std + label_mean
        y_pred.extend(preds)
        y_true.extend(raw_labels.numpy())
...
def main():
    ...
    n_taxis    = len(taxi_vocab)
    print(f"  taxi vocab size: {n_taxis}")
    ...
    # Normalise labels using train-set statistics
    ...
    # Model
    model = DeepTTE(n_taxis=n_taxis).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  DeepTTE params: {n_params:,}")

    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    loss_fn   = nn.HuberLoss(delta=1.0)   # delta=1 in normalised space ≈ 1 std
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for coords, cdist, attr_ids, dist_n, labels, lengths, _ in train_loader:
            ...
            attr_ids = attr_ids.to(device)
            dist_n   = dist_n.to(device)
            labels   = labels.to(device)   # normalised
            lengths  = lengths.to(device)

            optimizer.zero_grad()
            preds = model(coords, cdist, attr_ids, dist_n, lengths)
            loss  = loss_fn(preds, labels)  # loss in normalised space
            loss.backward()
        ...
        val_mae_val = val_m["mae_s"]

        improved = val_mae_val < best_val_mae
        if improved:
        ...
        print(f"  ep {epoch:3d}  loss {avg_loss:.2f}  val MAE {val_mae_val:.2f}s{marker}")

        if patience_cnt >= args.patience:
            print(f"  Early stop at epoch {epoch} (patience {args.patience} exceeded).")
            break
